package ru.yandex.practicum.controller.integration;
...
public class RestCollectorControllerIntegrationTest {
    ...
    void setupKafkaMock() {
        ...
        clearInvocations(avroKafkaTemplate);
        ...
        doReturn(future).when(avroKafkaTemplate).send(anyString(), anyString(), any(SpecificRecord.class));
    }
            ...
            ScenarioAddedEvent.builder()
                    ...
                    .conditions(List.of(
                            new ScenarioCondition("sensor-1", ConditionType.TEMPERATURE,
                                    ...
                                    // Берем рандом для проверки корректности работы десериализации
                                    RANDOM.nextBoolean()
                                            ? new BoolValue(true)
                                            : new IntValue(RANDOM.nextInt(100)))
                    ))
                    .actions(List.of(
    ...
    void shouldProcessSensorEventsSuccessfully() {
        ...
        sensorEvents.forEach(event -> {
            verify(avroKafkaTemplate, times(1))
